...
static u8 max_report_id;
static struct report_id_map_t *rid_map;
...
static u8 firmware_latest[] = {0x11, 0xc1};	/* version, build_version */
...
/*	object = mxt_get_object_info(data, SPT_TIMER_T61);*/
	get_object_info(data,
			SPT_TIMER_T61, &size, &obj_address);
	buf[1] = cmd;
	buf[2] = mode;
	buf[3] = msPeriod & 0xFF;
...
void mxt_t8_autocal_set(struct mxt_data *data, u8 mstime)
{
	...
	if (mstime) {
		data->pdata->ucfCheck_AutoCal = 1;
		dev_info(&data->client->dev,
			"T8 Autocal Enabled %d\n", mstime);
	} else {
		data->pdata->ucfCheck_AutoCal = 0;
		dev_info(&data->client->dev,
...
static void equalize_coordinate(bool detect, u8 id, u16 *px, u16 *py)
{
	...
	if (g_ucFilterNum == 1) {
		temp = (Square(pre_x[id] - *px) + Square(pre_y[id] - *py));
		if (temp <= Square(20)) {
		...
		*px = \
			(pre_x[id] * g_ucFilterConst1 + *px) \
				/ (g_ucFilterConst1 + 1);
		*py = \
			(pre_y[id] * g_ucFilterConst1 + *py) \
				/ (g_ucFilterConst1 + 1);

		if (abs(*px - pre_x[id]) > g_ucFilterConst2)
			*px = \
			(pre_x[id] * g_ucFilterConst3 + *px) \
				/ (g_ucFilterConst3 + 1);
		if (abs(*py - pre_y[id]) > g_ucFilterConst2)
			*py = \
			(pre_y[id] * g_ucFilterConst3 + *py) \
				/ (g_ucFilterConst3 + 1);
	} else if (g_ucFilterNum == 3) {
		if (nn[id] > 0) {
			temp = (Square(pre_x[id] - *px) +\
				Square(pre_y[id] - *py));
			if (temp > g_ucFilterConst2)
				if (nn[id] > 1)
					nn[id] >>= 1;
				/* 4->2->1 */
	...
	printk(KERN_INFO "[TSP] ID[%d] P[%d, %d] C[%d, %d] F[%d, %d]\n",
		id, pre_x[id], pre_y[id], cur_x, cur_y, *px, *py);
	printk(KERN_INFO "[TSP] M[%d, %d, %d, %d]\n",
			g_ucFilterNum, g_ucFilterConst1,
			g_ucFilterConst2, g_ucFilterConst3);
		...
		if (data->finger_mask  & ~(1<<2)) {
			if (tch_area > atch_area) {
				if (tch_area - atch_area <= 3) {
					...
					pr_info("TCHAREA=%d, ATCHAREA=%d,
					finger_mask=%d\n",
					tch_area, atch_area, data->finger_mask);
					mxt_calibrate_chip(data);
				}
				} else {
					if (atch_area > 0) {
	...
	if (object_type == SPT_TIMER_T61) {
		...
		if (object_type == TOUCH_MULTITOUCHSCREEN_T9) {
			...
			if (msg[1] & RELEASE_MSG_MASK) {
				...
				data->fingers[id].x = \
				(u16)((data->fingers[id].x * 480) / 4096);
				/* 800 -> 480 */
				data->fingers[id].y = \
				(u16)((data->fingers[id].y * 800) / 4096);
				/* 480 -> 800 */
#endif
...
#endif
#ifdef STYLUS_FILTER_TEST
			equalize_coordinate(0, id, &data->fingers[id].x,
						&data->fingers[id].y);
#endif
			}
